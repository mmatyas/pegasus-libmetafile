...
class test_Metafile : public QObject {
    Q_OBJECT
...
void test_Metafile::onAttributeFound(const metafile::Entry& entry)
{
    m_entries.emplace_back(metafile::Entry {entry.line, entry.key, entry.values});
}

void test_Metafile::onError(const metafile::Error& error)
{
    qWarning().noquote() << QObject::tr("line %1: %2")
        .arg(QString::number(error.line), error.message);
}

void test_Metafile::readStream(QTextStream& stream)
{
...
void test_Metafile::merge_lines()
{
...
void test_Metafile::merge_lines_data()
{
...
QTEST_MAIN(test_Metafile)
#include "test_Metafile.moc"
